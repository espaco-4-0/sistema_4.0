...
function FooterItem({ icon: Icon, title, desc, href, external = false }: FooterItemProps) {
    ...
    return (
        <li>
            ...
            >
                <div className="bg-yellow-primary text-black p-4 rounded-2xl shrink-0 group-hover:scale-105 transition-transform">
                    <Icon size={20} />
                </div>
                <div>
                    <p className="font-medium text-base leading-tight">{title}</p>
                    <p className="text-sm text-gray-400 mt-1 max-w-[200px] break-words">{desc}</p>
                </div>
            </Link>
        </li>
    );
