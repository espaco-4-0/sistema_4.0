...
import { getVisitRequests } from "@/src/ui/lib/visit-requests-storage";
...
export function AgendaVisitasAdmin() {
    ...
    useEffect(() => {
        setRequests(getVisitRequests());
        setCalendarEvents(getUnifiedCalendarEvents());
    }, []);
    ...
    const metrics = useMemo(() => {
        const aguardandoEmail = requests.filter((request) => request.processStage === "aguardando_email").length;
        const emAnalise = requests.filter(
            (request) => request.processStage === "email_recebido" || request.processStage === "documentacao_em_analise"
        ).length;
        const aguardandoIfal = requests.filter(
            (request) => request.processStage === "aguardando_aprovacao_ifal"
        ).length;
        const aprovados = requests.filter((request) => request.status === "aprovado").length;
    ...
    function refresh(setter: () => VisitRequest[]) {
        setRequests(setter());
        setCalendarEvents(getUnifiedCalendarEvents());
    }
                            ...
                            <TabsPanels>
                                <TabsPanel value="solicitacoes" className="space-y-3 pt-2">
                                    <div className="text-xs text-gray-500 px-0.5">
                                        Mostrando {paginatedRequests.length} de {sortedRequests.length} resultado(s)
                                    </div>
                                            ...
                                            <ul className="space-y-2">
                                                {dayEvents.map((event) => (
                                                    ...
                                                    >
                                                        ...
                                                        {format(event.start, "HH:mm")} as {format(event.end, "HH:mm")}
                                                    </li>
                                                ))}
                                            </ul>
                                        )}
